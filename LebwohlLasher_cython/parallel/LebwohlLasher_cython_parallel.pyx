...
#=======================================================================
cdef void savedat(np.ndarray[np.float64_t, ndim=2] arr,
                  int nsteps,
                  float Ts,
                  float runtime,
    ...
    filename = "cy_par-{:d}_size-Output-{:s}.txt".format(nmax, current_datetime)
...
#=======================================================================
cdef inline double one_energy_mv(double[:, :] arr, int ix, int iy, int nmax) nogil:
    """
    Arguments:
      arr (float(nmax,nmax)) = array that contains lattice data;
...
cpdef double one_energy(np.ndarray[np.float64_t, ndim=2] arr, int ix, int iy, int nmax):
    ...
    """
    cdef double[:, :] mv = arr
    # We call the nogil kernel (but from a GIL context is fine)
    return one_energy_mv(mv, ix, iy, nmax)


#=======================================================================
cdef double all_energy_mv(double[:, :] arr, int nmax) nogil:
    """
    Arguments:
      arr (float(nmax,nmax)) = array that contains lattice data;
      nmax (int) = side length of square lattice.
...
cdef float MC_step(np.ndarray[np.float64_t, ndim=2] arr, float Ts, int nmax):
    ...
    for i in range(nmax):
        for j in range(nmax):
            ...
            iy = yran[i,j]
            ang = aran[i,j]
            en0 = <float>one_energy(arr,ix,iy,nmax)
            arr[ix,iy] += ang # not to be parallelised as this would be a race condition
            en1 = <float>one_energy(arr,ix,iy,nmax)
            if en1<=en0:
                accept += 1
            else:
                # Now apply the Monte Carlo test - compare
                # exp( -(E_new - E_old) / T* ) >= rand(0,1)
                boltz = <float>exp( -(en1 - en0) / Ts )
