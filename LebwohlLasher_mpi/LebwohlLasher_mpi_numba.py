...
# This is needed as the two exist using different memory models:
# - global works on a fully gathered nmax x nmax lattice (serial-style periodic modulo)
# - local works on a subdomain plus ghost cells (periodicity via halo exchange)
...
def one_energy_local(arr, ix, iy):
    ...
    diff = center - arr[iy, ix + 1]
    en += 0.5 * (1.0 - 3.0 * (np.cos(diff) ** 2))

    diff = center - arr[iy, ix - 1]
    en += 0.5 * (1.0 - 3.0 * (np.cos(diff) ** 2))

    diff = center - arr[iy + 1, ix]
    en += 0.5 * (1.0 - 3.0 * (np.cos(diff) ** 2))
...
# ============================================================
@njit
def local_energy_sum_nb(arr):
    """
    Arguments:
      arr (float(ny+2,nx+2)) = local array including ghost cells.
    Description:
      Numba-accelerated local energy sum. Requires ghost cells to be
      correct (halo_exchange called in Python before this runs).
    Returns:
      (float) = local reduced energy contribution (not reduced across ranks).
    """
    ny = arr.shape[0] - 2
    nx = arr.shape[1] - 2
    total = 0.0
    for iy in range(1, ny + 1):
        for ix in range(1, nx + 1):
            total += one_energy_local(arr, ix, iy)
    return total
...
def all_energy(arr):
    ...
    # Numba does the heavy local sum; MPI reduces across ranks.
    en_local = local_energy_sum_nb(arr)
    return comm.allreduce(en_local, op=MPI.SUM)

# ============================================================
@njit
def local_Qab_nb(arr):
    """
    Arguments:
      arr (float(ny+2,nx+2)) = local array including ghost cells.
    Description:
      Compute the unnormalised local Q tensor contribution:
        Sum_{sites} (3 v v^T - I)
      This is Numba-safe. The MPI allreduce and normalisation are done in Python.
    Returns:
      Qab_local (float(3,3)) = local contribution to Q tensor.
    """
    ny = arr.shape[0] - 2
    nx = arr.shape[1] - 2
    ...
    for iy in range(1, ny + 1):
        for ix in range(1, nx + 1):
            ...
            Q[1, 0] += 3.0 * s * c
            Q[1, 1] += 3.0 * s * s
            # third component is 0 so only identity subtraction affects Q[2,2]
...
def get_order(arr, nmax):
    ...
    Description:
      ...
      Note: MPI calls (allreduce) and eigendecomposition remain in Python.
            Only the local Q tensor accumulation is Numba-accelerated.
    Returns:
      max(eigenvalues(Qab)) (float) = order parameter for lattice.
    """
    Qab_local = local_Qab_nb(arr)
...
# ============================================================
@njit
def mc_sweep_nb(arr, Ts, xran, yran, aran, uran):
    """
    Arguments:
      arr (float(ny+2,nx+2)) = local lattice data including ghost cells;
      Ts (float) = reduced temperature (range 0 to 2).
      xran, yran (int arrays) = random interior coordinates (1..nx, 1..ny)
      aran (float array) = proposed angle changes
      uran (float array) = uniform random numbers for acceptance test
    Description:
      Numba-accelerated Monte Carlo sweep over local sites.
      Randomness is provided by the caller (Python) to keep MPI behaviour simple.
    Returns:
      (int) = local number of accepted moves.
    """
    ny = arr.shape[0] - 2
    nx = arr.shape[1] - 2
    accept = 0
...
def MC_step(arr, Ts):
    ...
    #
    # Pre-compute random numbers in Python (simple + avoids Numba RNG pitfalls).
    # "scale" sets the width of the distribution for the angle changes.
    #
    scale = 0.1 + Ts

    xran = np.random.randint(1, nx + 1, size=(ny, nx)).astype(np.int64)
    yran = np.random.randint(1, ny + 1, size=(ny, nx)).astype(np.int64)
    aran = np.random.normal(loc=0.0, scale=scale, size=(ny, nx)).astype(np.float64)
    uran = np.random.random(size=(ny, nx)).astype(np.float64)

    accept_local = mc_sweep_nb(arr, Ts, xran, yran, aran, uran)

    # reduce acceptance count across all ranks
    total_accept = comm.allreduce(accept_local, op=MPI.SUM)
...
def main(program, nsteps, nmax, temp, pflag):
    ...
    # ------------------------------------------------------------
    # Numba warm-up (compile on each rank) so timing excludes JIT cost
    halo_exchange(lattice)
    ny = lattice.shape[0] - 2
    nx = lattice.shape[1] - 2
    xran = np.random.randint(1, nx + 1, size=(ny, nx)).astype(np.int64)
    yran = np.random.randint(1, ny + 1, size=(ny, nx)).astype(np.int64)
    aran = np.random.normal(loc=0.0, scale=0.1 + temp, size=(ny, nx)).astype(np.float64)
    uran = np.random.random(size=(ny, nx)).astype(np.float64)
    _ = mc_sweep_nb(lattice, temp, xran, yran, aran, uran)
    _ = local_energy_sum_nb(lattice)
    _ = local_Qab_nb(lattice)
    comm.Barrier()
    # ------------------------------------------------------------
